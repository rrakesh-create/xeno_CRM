...
import { X, User, Mail, Phone, MapPin, CreditCard, ShoppingBag, Activity } from 'lucide-react';

export default function ShopperDrawer({ shopperId, onClose }) {
  ...
  const fetchShopper = async () => {
    try {
      const res = await getCustomer(shopperId);
      setData(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const runDiagnosis = async () => {
    setLoadingDiag(true);
    try {
      const res = await aiDiagnose(shopperId);
  ...
  return (
    <>
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 40 }} onClick={onClose}></div>
      <div className="shopper-detail" style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '600px', maxWidth: '90vw', background: 'var(--bg-dark)', zIndex: 50, overflowY: 'auto', padding: '24px', boxShadow: '-4px 0 15px rgba(0,0,0,0.1)' }}>
        <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'flex-end' }}>
          <button className="btn-primary" style={{ background: 'transparent', color: 'var(--text-secondary)', border: 'none', padding: '8px' }} onClick={onClose}>
            <X size={24} />
          </button>
        </div>
